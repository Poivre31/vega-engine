...
class Rn : public std::array<double, N> {
   public:
    [[nodiscard]] friend Rn operator*(double a, const Rn& X) {
        Rn Y;
    ...
    [[nodiscard]] friend Rn operator*(const Rn& X, double a) { return a * X; }

    [[nodiscard]] friend Rn operator+(const Rn& X, const Rn& Y) {
        Rn Z;
        for (size_t i = 0; i < N; i++) {
            Z[i] = X[i] + Y[i];
        }
    ...
    [[nodiscard]] friend bool operator==(const Rn& X, const Rn& Y) {
        for (size_t i = 0; i < N; i++) {
            if (X[i] != Y[i]) {
                return false;
            }
    ...
    Rn apply(const std::function<double(double)>& f) {
        Rn Y;
        for (size_t i = 0; i < N; i++) {
            Y[i] = f(this->at(i));
        }
