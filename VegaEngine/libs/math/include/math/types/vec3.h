...
class vec3 {
 public:
  ...
  /** Checks if x, y and z are in the [ @param lower, @param upper ] range
   * (bornes excluded) */
  [[nodiscard]] constexpr bool is_strictly_within(vec3 lower, vec3 upper) noexcept;
  ...
  [[nodiscard]] constexpr friend vec3 operator+(const vec3& u, const vec3& v) noexcept {
    vec3 out;
    out.x = u.x + v.x;
    out.y = u.y + v.y;
    out.z = u.z + v.z;
  ...
  [[nodiscard]] constexpr friend vec3 operator*(const vec3& v, T a) noexcept { return a * v; }
  ...
  {
    if (a == 0) [[unlikely]] {
      throw std::runtime_error("Division by zero (trying to divide vec3 by null number ");
    }
    vec3 out;
    out.x = u.x / a;
    out.y = u.y / a;
