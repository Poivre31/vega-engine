...
class vec2 {
 public:
  ...
  /** Checks if x, y and z are in the [ @param lower, @param upper ] range
   * (bornes excluded) */
  [[nodiscard]] constexpr bool is_strictly_within(vec2 lower, vec2 upper) noexcept;
   ...
   * [VegaMath] */
  void print(const std::shared_ptr<spdlog::logger>& console = console::get(consoles::math)) const;

  /** Prints the vector's components preceded by @param message using given
   * spdlog logger, defaults to [VegaMath] */
  void print(
      const std::string& message,
      const std::shared_ptr<spdlog::logger>& console = console::get(consoles::math)
  ) const;
  ...
  [[nodiscard]] constexpr friend vec2 operator+(const vec2& u, const vec2& v) noexcept {
    vec2 out;
    out.x = u.x + v.x;
    out.y = u.y + v.y;
    return out;
  ...
  [[nodiscard]] constexpr friend vec2 operator*(const vec2& v, T a) noexcept { return a * v; }
  ...
  {
    if (a == 0) [[unlikely]] {
      throw std::runtime_error("Division by zero (trying to divide vec2 by null number ");
    }
    vec2 out;
    out.x = u.x / a;
    out.y = u.y / a;
...
struct fmt::formatter<vega::math::vec2<T>> : fmt::formatter<T> {
  auto format(vega::math::vec2<T> v, format_context& ctx) const -> decltype(ctx.out()) {
