...
TEST(TestConsole, TestLogLevels) {
  ...
  EXPECT_NO_FATAL_FAILURE(
      auto vega_console = console::get(); vega_console->set_level(level::warn);
...
TEST(TestConsole, TestConsoleAccess) {
  EXPECT_NO_FATAL_FAILURE(
      auto vega_console = console::get(); vega_console = console::create("vega-engine");
      auto test_conole                                 = console::create("Test");
      auto test2                                       = console::get("Test");
      test_conole->debug("Working 1");
      test2->debug("Working 2");
      console::get("Lame")->critical("This should not be visible");
      console::create("vega-engine")->critical("This should not be visible");
  );
}
