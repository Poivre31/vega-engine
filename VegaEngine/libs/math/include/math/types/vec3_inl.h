...
constexpr size_t vec3<T>::norm_L0() const noexcept {
  return static_cast<size_t>(x != 0) + static_cast<size_t>(y != 0) + static_cast<size_t>(z != 0);
}
...
constexpr bool vec3<T>::is_within(T lower, T upper) noexcept {
  return math::is_within(x, lower, upper) && math::is_within(y, lower, upper)
         && math::is_within(z, lower, upper);
}

template <math::numeric T>
constexpr bool vec3<T>::is_within(vec3 lower, vec3 upper) noexcept {
  return math::is_within(x, lower.x, upper.x) && math::is_within(y, lower.y, upper.y)
         && math::is_within(z, lower.z, upper.z);
}

template <math::numeric T>
constexpr bool vec3<T>::is_strictly_within(T lower, T upper) noexcept {
  return math::is_strictly_within(x, lower, upper) && math::is_strictly_within(y, lower, upper)
