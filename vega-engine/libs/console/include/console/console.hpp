 ...
 * [vega-engine] console is always available and the default return to `sget`
 *
 */
class console {
 public:
   ...
   */
  [[nodiscard]] static bool exists(const std::string& name = "vega-engine") noexcept;
  ...
  */
  ...
  /**
   * @brief Returns one of the default consoles such as [vega-engine] or
   * [VegaMath].
   */
  [[nodiscard]] static vega_console
  get(consoles console = consoles::engine, bool silence = silence_console_management) noexcept;

  /** @brief Silences all logs under @param level for all library consoles
   * For example, after calling with level 'info', every logs to [vega-engine] or [VegaMath] with
